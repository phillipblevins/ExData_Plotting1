

hpc <- read.csv("household_power_consumption.txt", sep=";", na.strings=c("","?","NA"))

twodayhpc <- subset(hpc, hpc$Date=="1/2/2007" | hpc$Date=="2/2/2007")

twodayhpc$Datetime <- as.POSIXct(paste(as.Date(twodayhpc$Date, format="%d/%m/%Y"), twodayhpc$Time))


png(filename = "plot4.png", width = 504, height = 504)

par(mfrow=c(2,2),mar=c(4,4,2,1))



plot(twodayhpc$Global_active_power ~ twodayhpc$Datetime, type = "l",  ylab = "Global Active Power (kilowatts)", xlab = "")




plot(twodayhpc$Voltage ~ twodayhpc$Datetime, type = "l",  ylab = "Voltage", xlab = "datetime")


plot(twodayhpc$Sub_metering_1 ~ twodayhpc$Datetime, type = "l",  ylab = "Energy Sub metering", xlab = "")
lines (twodayhpc$Sub_metering_2 ~twodayhpc$Datetime, col="red")
lines (twodayhpc$Sub_metering_3 ~twodayhpc$Datetime, col="blue")
legend("topright", col = c("black", "red", "blue"), lty = 1, lwd = 2, 
       legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"))




plot(twodayhpc$Global_reactive_power ~ twodayhpc$Datetime, type = "l",  ylab = "Global_reactive_power", xlab = "datetime")

dev.off()